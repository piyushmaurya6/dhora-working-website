...
onVariantChange_fn = function (event) {
  console.log(">>>,variant");
  try {
    let variant1 = event.detail.variant;
    console.log(">>>", variant1);
    if (variant1?.available) {
      let breezeButtonContainers = document.getElementsByClassName(
        "product-form_breezeButton"
      );
      let breezeButtonContainer1 = document.getElementsByClassName(
        "product-form_breezeButton"
      )[breezeButtonContainers.length - 1];
      console.log(">>>", breezeButtonContainer1);
      breezeButtonContainer1
        .getElementsByTagName("breeze-button")[0]
        .setAttribute(
          "selectedproductvariantdata",
          btoa(JSON.stringify(variant1))
        );
    }
  } catch (e) {}
